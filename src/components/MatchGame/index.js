...
import ThumbnailItem from '../ThunbnailItem'
...
class MatchGame extends Component {
  ...
  playAgain = () => {
    this.setState(prevState => ({
      showScoreCard: !prevState.showScoreCard,
      score: 0,
      time: 60,
      activeTabId: tabsList[0].tabId,
      imageList: imagesList[0],
  ...
  onClickThumbnail = id => {
    const {imageList, score} = this.state
    const {data} = this.props
    const shuffledImagesList = data.sort(() => Math.random() - 0.5)
    if (id === imageList.id) {
      this.setState({
        score: score + 1,
        imageList: shuffledImagesList[0],
      })
    } else {
  ...
  renderGameCard = () => {
    const {activeTabId, imageList} = this.state
    const searchResults = imagesList.filter(
      item => item.category === activeTabId,
    )
    ...
    return (
      <div className="game-container">
        <img src={imageList.imageUrl} className="img" alt="match" />
        ...
        <ul className="thumbnails-list">
          {searchResults.map(eachItem => (
            <ThumbnailItem
              eachItem={eachItem}
              key={eachItem.id}
              isMatched={eachItem.id === imageList.id}
  ...
  renderGame = () => {
    const {time, showScoreCard} = this.state
    if (time === 0) {
      clearInterval(uniqueId)
      return this.renderScoreCard()
    }
    return showScoreCard ? this.renderScoreCard() : this.renderGameCard()
  }
